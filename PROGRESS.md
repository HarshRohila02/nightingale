...
**Last updated:** 2026-09-24 · by Claude (🎯 W3 walking skeleton reached: the real graph behind `scripts/demo.py`; earlier today 2a ✅, EXP-005, and the `docs/05` amendment 3 proposal for the team) · **Last verified commit:** `51169f5`
...
| **Health** | 407 tests (401 + 3 for the demo, 2 for the Neo4j fallback, 1 for the explainer). Locally 398 pass and the 9 live Neo4j tests skip unless `NEO4J_TEST_DOTENV=1` (with it all 9 passed against Aura on 2026-09-19) · **CI green on GitHub at `51169f5`**: 368 passed, 33 skipped (CI has no `data/` or `models/`) · the Colab-trained B0/B1 models reproduce their validate metrics to within 3 × 10⁻¹² |
...
resolve at all (most likely paused since ~2026-09-22): the owner resumes it with **Play** when
Aura is wanted. **2a is done (EXP-005):** the graph score is a naive-Bayes
...
| CI (GitHub Actions) | ✅ green on every push so far (latest verified: `51169f5`, 2026-09-24: 368 passed, 33 skipped — CI has no `data/` or `models/`, so the data-gated tests skip) · Python 3.11 · since 2026-09-19 the job also starts a throwaway `neo4j:5-community` container for the live Neo4j tests |
