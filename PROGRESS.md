...
**Last updated:** 2026-09-25 · by Claude (amendment 3 applied; EXP-019 scored on its masks: the original B1 misses the safety targets at 25%, the `+aug` models meet them; the configured ranker is now B1-LR′+aug; A-7 settled) · **Last verified commit:** `0f47d67`
...
| **Health** | 683 tests (667 + 16 for amendment 3's digests, A-7 and the configured ranker's golden cases). Locally 665 pass, **9 fail because Windows' Application Control now blocks scikit-learn** (§7; CI runs them), and the 9 live Neo4j tests skip unless `NEO4J_TEST_DOTENV=1` (with it all 9 passed against Aura on 2026-09-19 and again on 2026-09-24) · **CI green on GitHub at `5ff4573`**: 629 passed, 38 skipped (CI has no `data/` or `models/`) · the Colab-trained B0/B1 models reproduce their validate metrics to within 3 × 10⁻¹² |
...
## 7. 🧰 Environment state — verified 2026-09-18 (Local Python row: 2026-09-19; CI, Heavy compute and Data rows: 2026-09-20; Neo4j row: 2026-09-24)

| Item | State |
|---|---|
| Git | clean · the repository is **public** on GitHub, so cloud notebooks clone it without a token |
| CI (GitHub Actions) | ✅ green on every push so far (latest verified: `5ff4573`, 2026-09-25: 629 passed, 38 skipped — CI has no `data/` or `models/`, so the data-gated tests skip) · Python 3.11 · since 2026-09-19 the job also starts a throwaway `neo4j:5-community` container for the live Neo4j tests |
| Local Python | **3.11.9** in `.venv` (D-2) — light deps only: `requirements-dev.txt` (now incl. pandas, numpy, pyarrow, **networkx 3.6.1**) + huggingface_hub + **`neo4j` 5.28.6, `python-dotenv` 1.2.3** (2026-09-19, for the Neo4j store) + **scikit-learn 1.9.1, XGBoost 2.1.4, SciPy 1.17.1** (2026-09-19, for the ML baselines); all now in `requirements-dev.txt`. Installs are per task (D-4). **2026-09-25: Windows now blocks scikit-learn** ("DLL load failed while importing _openmp_helpers: An Application Control policy has blocked this file"; the file is unchanged since its install on 2026-09-19), so the 9 tests that fit a model with it fail on the laptop; CI (Linux) still runs them, and nothing that scores a saved model needs scikit-learn. Allowing it is a Windows security setting, the owner's call |
