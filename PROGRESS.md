...
**Last updated:** 2026-09-25 · by Claude (amendment 3 applied; EXP-019 scored on its masks: the original B1 misses the safety targets at 25%, the `+aug` models meet them; the configured ranker is now B1-LR′+aug; A-7 settled) · **Last verified commit:** `34d9f40`
...
| **Next action** | **Claude, unblocked:** 2c, the fusion (D-10 settled), with the configured B1-LR′+aug and knowing the red flags add little at reduced evidence (EXP-019); 1e, the FastAPI endpoints; 2b's calibration, including hand-written input. **The team:** **E-1**, the errata to amendment 3, now with item 17 (which B1 the reduced-evidence claims compare against: EXP-019 shows it decides them) · **A-9** · **T-6**, tuning parity · **A-8** and the clinical review of the 17 representative answers (gate 1's last item before anything deep is trained). **The owner:** run `notebooks/colab_b1_seeds.ipynb` on Colab (XGBoost's seeds 43–46, EXP-020; the owner chose Colab on 2026-09-25, D-7; about 50–60 min, `docs/11` §4.3), then unzip it into `models/nightingale_b1_seeds/`; ~~decide whether to allow scikit-learn past Windows' Application Control (§7)~~ decided 2026-09-25: Smart App Control stays on; the tests skip locally (§7). **The user:** GC-005; whether the bundles move to `models/b0_b1/`. |
| **Blocked on** | Anything deep (B1-DL) ← **A-8**, the team (gate 1's last item) · Phase 3 LLM ← D-5 (deferred) · university GPU access (external, R-14). **Every training or tuning run, and any job over ~5 min, waits for the user to say where** (D-7) |
...
| **Health** | 683 tests (667 + 16 for amendment 3's digests, A-7 and the configured ranker's golden cases). Locally 665 pass and 18 skip: the 9 that need scikit-learn, **which Windows' Smart App Control blocks on the laptop** (§7; they skip only for that error, and CI runs them), and the 9 live Neo4j tests unless `NEO4J_TEST_DOTENV=1` (with it all 9 passed against Aura on 2026-09-19 and again on 2026-09-24) · **CI green on GitHub at `b8c595a`**: 640 passed, 43 skipped (CI has no `data/` or `models/`) · the Colab-trained B0/B1 models reproduce their validate metrics to within 3 × 10⁻¹² |
...
**R-18's "asked" channel is built (2026-09-24), and EXP-018 (2026-09-25) retrained B1 with it:**
the retrained models no longer answer atrial fibrillation to short input, and the masked copies
do that alone; what the channel adds waits for masked validate (amendment 3). ~~They are
overconfident on short input (GC-001: unstable angina 0.98–1.00), and the configured ranker
stays B1-LR.~~ **EXP-019 (2026-09-25):** on masked validate they are calibrated against DDXPlus,
but near-certain on hand-written cases (GC-001: unstable angina 0.98–1.00); **the configured
ranker is now B1-LR′+aug**. The bundles are in `models/nightingale_b1_asked/`. `EvidenceEncoder(asked_channel=True)`
...
whole-question denials now reach a model that has the channel. The channel is constant at full
evidence, so the retrain trains B1 on masked copies, with and without it. ~~No model with the
channel exists yet, so the configured ranker is unchanged.~~ The channel models exist since
EXP-018, and B1-LR′+aug is the configured ranker since EXP-019. **🎯 The W3 walking skeleton is
reached (2026-09-24):**
`scripts/demo.py` runs on the real graph (`--graph`, default `graph.backend`), opens Aura
read-only, and falls back to the local build, saying so. On 2026-09-24 Aura's host name did not
resolve at all (most likely paused since ~2026-09-22); the owner resumed it the same day, and the
demo is now served by Aura. A Free instance pauses after 72 h unused. **2a is done (EXP-005):** the graph score is a naive-Bayes
...
anything until the "asked" channel lands. **B0 and B1 are trained** (the owner's
...
decision D-10 for the team** (§3). On 2026-09-19 the user created the
...
**Task (2026-09-25):** (1) the Colab notebook for XGBoost's seeds 43–46 (`docs/05` §6; the owner
chose Colab, D-7), committed first so the owner can run it; (2) then 2c, the fusion layer.

**Done:** the notebook, its runbook entry and EXP-020's planned row (committed with this line).

**Remaining:** 2c's design, code and tests. Any fusion weight sweep on project data is tuning: ask
where before running it (D-7).

**If interrupted:** nothing has been trained or tuned here; the notebook only runs on Colab.
...
| **A-9** | How the encoder's "asked" channel treats the questions a patient did **not** list when a mask shortens their history (`docs/02` §9). *Sent with amendment 3 but not named in its approval (2026-09-25)* | asked at the same share as the listed ones (current setting) · never asked | **The current setting.** Counting only the kept listed questions as asked would leak the label (DDXPlus lists a default answer only for conditions that ask it) and leave masked patients almost no denials. The kept tokens and the digest are the same either way; EXP-019 records the asked sets' digest, so a change re-runs cleanly | The ′ models' reduced-evidence input (EXP-019), and the masked training copies of the configured ranker, B1-LR′+aug |
| **T-6** | **Tuning parity** (amendment-3 proposal §7, decision 6; not in the approved text): how much tuning each ML model may have before H6 is judged | no tuning beyond early stopping on a 10% training holdout, for every model · an equal, declared validation-tuning budget for each | **No tuning beyond early stopping**, what EXP-004 and EXP-018 did. An untuned B1 against a tuned B1-DL would not be a fair H6 | H6, before B1-DL is trained |
| **E-1** | **The errata to amendment 3** (`docs/proposals/05-amendment-3-errata.md`): contradictions an independent review found inside the approved wording, such as §4's "B1-XGB is never retrained under the same label" against §6's "the gap is closed when B1-XGB is next retrained" | approve the corrections · amend them · leave the text as it is | **Approve**: several decide how H1-R, H2-R and H6 are judged, so they should be fixed before any fusion or deep result exists | How 2c and H6 are judged |
...
ranker with **real** KG-matched supporting findings, end to end · CI green. *(W3 ✅ 2026-09-24, in
`scripts/demo.py`; the phase still needs 1d and 1e.)*
...
**1c — Baseline ML ranker · ✅ 2026-09-23** · P2
...
- [x] Translate hand-authored cases into DDXPlus tokens, the seam the ranker needs
  ...
  Smoke-testing it produced **EXP-017** and **R-18** — `cb8ddc6`
- [x] `ConditionRanker` replacing `ConstantRanker` (`src/ml/ranker.py`): `ModelRanker` wraps
  ...
  four golden cases hold on the real graph. 19 tests — `3436d0b`
- [x] The golden cases re-run with **red flags off**, so the ranking is under test rather than
  ...
  each condition's evidence-set size) so that it fails loudly the day 2a fixes it — `3436d0b`
- [x] `src/eval/reports.py`: the scoring helpers moved out of `scripts/train_baselines.py`, so that the baseline job and the deep job write a `metrics.json` that means the same thing field for field — otherwise the second job reimplements it slightly differently and the comparison is quietly invalid. `compare()` refuses two systems scored on different cases, because McNemar is paired. The script's end-to-end test re-runs it as a subprocess, so the move verifies itself — `4698759`
...
- [x] Re-verify golden cases against real components — **fix the component, not the expectation**. All four hold on the real graph and the real model: with red flags on (`scripts/check_crosswalk.py`), with them off (`tests/test_ranker.py`, since `3436d0b`; GC-001 fixed by 2a in `868f837`, not by changing its expectation), and in the demo — `26a00ad`
- [x] 🎯 **The real graph behind `scripts/demo.py`, the W3 walking skeleton.** `--graph neo4j | networkx | stub` (default `graph.backend`), through `open_graph_from_config()` in `scripts/_config.py`; the case expanded through the crosswalk for the graph. Aura is opened **read-only** (`open_graph_store(..., sync=False)`): a stale copy is a fallback, never a write. **Found and fixed:** neo4j 5.28 raises a bare `ValueError` when the instance's host does not resolve, which crashed the demo instead of falling back. Its explanation keeps each question the graph matched, with the recorded finding that answered it beside it (docs/02 §5.2); a first attempt that relabelled the question as the finding was reverted before commit, since "pressure-type pain supports GERD" is not what the graph says. The template explainer names five unrecorded findings and counts the rest (MI has 24). 6 tests — `26a00ad`
...
- **2a** · P1 — KG scoring ~~(overlap → Personalised PageRank)~~ + reasoning paths · EXP-005.
  ...
  xfail promoted. B2 on validate (circular): top-1 0.923, Precision@3 0.756 — `868f837`
...
- **R-18, the "asked" channel** (the deep-ranker plan's R2) · code ✅ 2026-09-24; retrain on **Colab**
  (the user, 2026-09-25, D-7), run by the owner at `4583e91`, ✅ evaluated 2026-09-25 (EXP-018):
  the atrial-fibrillation answer to short input is gone from all four retrained models (the
  masked copies alone do it); full evidence unchanged; the retrained models are overconfident
  on short input; masked validate waits for amendment 3 — `5ff4573`. **EXP-019 (2026-09-25), on the approved masks:** at 25% the original B1 misses the safety targets (must-not-miss recall@3 0.877 XGB, 0.925 LR), every `+aug` model meets them, and the channel adds a small significant gain; the "overconfident on short input" of EXP-018 is narrowed (calibrated against DDXPlus on masked validate, ECE ≤ 0.011; near-certain on hand-written cases). **The configured ranker is now B1-LR′+aug** — `b8c595a`. `EvidenceEncoder(asked_channel=True)`: `E_nn=unasked`
  columns, 691 in all, fingerprint `ddd14019c66eff10`; off, the B1 models' `3a0d5a5e01d7f427`
  (now pinned by a test). `src/ml/evidence_masks.py`: the proposal's question rule, unchanged
  (`scripts/check_mask_rules.py` now uses it, and both digests reproduce), plus the unlisted
  questions asked at the same share (**A-9**) and `augment()` for masked training copies.
  `CaseTokens.asked` / `denials_asked`: GC-004's denials of exertional pain and diaphoresis now
  reach a model with the channel; its denied jaw/arm radiation cannot (the pain may radiate
  elsewhere). `open_ranker()` opens a model of either kind by its fingerprint.
  `scripts/train_baselines.py --asked-channel --augment` and `notebooks/colab_b1_asked.ipynb`
  (EXP-018): B1 +aug and B1′+aug, validate at full evidence only, plus a no-findings probe.
  A toy test shows the effect: a one-finding history stays open (0.50) where a denial decides
  (0.99); without the channel both read 0.98. **Found on the way:** `LogisticBaseline` gave
  probability 1 to every row of a *two*-class fit (the 13-class models were never affected);
  fixed — `4583e91`
- **XGBoost's seeds 43–46** (`docs/05` §6, amendment 3; EXP-020) · notebook ready 2026-09-25, **Colab** (the owner's choice, D-7): `notebooks/colab_b1_seeds.ipynb` trains seeds 43–46 of B1-XGB, B1-XGB+aug and B1-XGB′+aug, and seed 42 of B1-XGB again as a check against EXP-004 (`docs/11` §4.3). The seed-42 models stay the ones used; the laptop scores every seed at the three levels once the bundles are back — → pending
- **2b** · P2 — Calibration (Platt vs isotonic) + SHAP · EXP-007. **EXP-018:** the retrained
  models give 0.98–1.00 to one condition on the golden cases' 5–13 findings, so calibrate on
  short input too, not only at full evidence. B1's raw scores already have ECE 0.0001 on validate (EXP-004), so calibration matters mainly for fused and reduced-evidence scores (D-10)
...
- **2d** · P3 — Red-flag rules on real concepts + safety layer v1 · EXP-008.
  ...
  **0.823** [0.818, 0.829], under the 0.95 target (pneumothorax 0.33, myocarditis 0.64, Boerhaave
  ...
  3,000 validate patients needed no repair. `scripts/check_red_flags.py`; 66 tests — `9986024`.
  **A-7 settled 2026-09-25** (delegated to Claude): a three-judge panel over all 104
  flag/condition pairs, majority adopted (`APPROPRIATE`, `docs/04` §3; told not to read results,
  but not fully blind to firing rates); precision 0.848 [0.845, 0.852] (strict 0.512). **At reduced evidence the rules collapse** (sensitivity 0.327 at 50%, 0.135 at 25%,
  EXP-019) — `b8c595a`
...
| 2026-09-25 | **`docs/05` amendment 3 approved** (v1.3): **D-10 (b)** — every system and ablation is also scored at 50% and 25% of each patient's evidence, masked by question (§3.7), digest recorded before scoring; **D-11 (a)** — the deep ranker B1-DL (`+aug`, seed 42) is A0's ML component, with A7 swapping B1 back in; B1 reported as B1-XGB and B1-LR, and every claim against B1 must hold against both; seeds 42–46. Applied as written, with the dates filled and §9 brought up to date (EXP-008, EXP-018); errors found in the approved wording go to the team as errata | the team, relayed by the owner · `docs/05` §9 |
| 2026-09-25 | **A-7: which red flags are clinically appropriate** — the ischaemic flags (MI, unstable angina) for the eight cardiac conditions on the ECG-and-troponin pathway, myocarditis and pulmonary edema for their overlapping cardiac conditions, and the flags for dissection, embolism, pneumothorax and Boerhaave for their own condition only. A three-judge panel's majority over all 104 pairs, told not to read results but not fully blind to firing rates (`APPROPRIATE` in `src/reasoning/red_flags.py`, `docs/04` §3) | the owner, delegated to Claude · `docs/02` §9 |
| 2026-09-25 | **The configured ranker becomes B1-LR′+aug** (`configs/config.yaml`): on EXP-019's masks the original B1-LR misses both safety targets at 25%, and B1-LR′+aug meets them. B1-XGB′+aug is a little better on the headline metrics, so 2c may revisit the family. The protocol's A0 uses B1-DL+aug once it exists | Claude, on EXP-019 (reversible) · `docs/08` EXP-019 |
| 2026-09-25 | **A-5 final: sudden onset stays `E_59` ≥ 8** (EXP-008: ≥ 6 would reach 54% of pneumothorax patients instead of 32%, from how DDXPlus draws onset values, not medicine) | the owner, taking Claude's recommendation · `docs/02` §9 |

---

## 7. 🧰 Environment state — verified 2026-09-18 (CI, Heavy compute and Data rows: 2026-09-20; Neo4j row: 2026-09-24; Local Python row: 2026-09-25)

| Item | State |
|---|---|
| Git | clean · the repository is **public** on GitHub, so cloud notebooks clone it without a token |
| CI (GitHub Actions) | ✅ green on every push so far (latest verified: `b8c595a`, 2026-09-25: 640 passed, 43 skipped — CI has no `data/` or `models/`, so the data-gated tests skip) · Python 3.11 · since 2026-09-19 the job also starts a throwaway `neo4j:5-community` container for the live Neo4j tests |
| Local Python | **3.11.9** in `.venv` (D-2) — light deps only: `requirements-dev.txt` (now incl. pandas, numpy, pyarrow, **networkx 3.6.1**) + huggingface_hub + **`neo4j` 5.28.6, `python-dotenv` 1.2.3** (2026-09-19, for the Neo4j store) + **scikit-learn 1.9.1, XGBoost 2.1.4, SciPy 1.17.1** (2026-09-19, for the ML baselines); all now in `requirements-dev.txt`. Installs are per task (D-4). **2026-09-25: Windows now blocks scikit-learn** ("DLL load failed while importing _openmp_helpers: An Application Control policy has blocked this file"; the file is unchanged since its install on 2026-09-19). **The cause is Windows 11's Smart App Control**, which is On (registry `HKLM\SYSTEM\CurrentControlSet\Control\CI\Policy`, `VerifiedAndReputablePolicyState` = 1). Its code-integrity log (event 3077, policy `{0283ac0f-fff1-49ae-ada1-8a933130cad6}`) shows it refusing `_openmp_helpers`, `_cd_fast` and `_pairwise_fast`: 3 of scikit-learn 1.9.1's 69 compiled files. It lets an unsigned file run only if Microsoft's reputation service knows it: numpy's and XGBoost's files are unsigned too, but load. Why it began on 2026-09-25 is not known (the log only reaches back to that evening); most likely Smart App Control left its evaluation mode. **Decided by the owner (2026-09-25): leave it on.** The 9 tests that need scikit-learn skip on this error only (`tests/sklearn_guard.py`), CI (Linux) runs them, training runs in the cloud anyway (D-7), and nothing that scores a saved model needs scikit-learn. Smart App Control has no per-file exception, and turning it off is a Windows security setting, the owner's alone |
...
| Neo4j | **AuraDB Free**: instance created by the user on 2026-09-19; `.env` filled. Login verified from the laptop without reading `.env`: Neo4j 5.27 (Aura). **It holds the KG since 2026-09-19** (`scripts/load_neo4j.py`): label `CardiacKG`, 129 nodes, 321 edges, fingerprint `de349201157a`, and nothing else. **Its home database is named after the instance id, not `neo4j`.** A Free instance pauses after 72 h unused (resume with **Play** in the console) and is deleted after 30 days paused. **2026-09-24: its host name did not resolve from the laptop** (other hosts did), most likely because it had been paused since ~2026-09-22; everything fell back to the local build as designed. **The owner resumed it the same day:** `load_neo4j.py --check` finds its copy current (fingerprint `de349201157a`, nothing written) and identical on the golden cases, the 9 live tests pass, and `scripts/demo.py` is served by Aura with nothing degraded |
...
| Data on disk (gitignored) | `data/raw/ddxplus/release_*.json` + **`validate.csv`** (87 MB) · `data/raw/bodhi_s/*.jsonl` · `data/interim/ddxplus_*.json`, `exp002_class_balance.json`, **`ddxplus_chestpain_validate.parquet`** (4.6 MB) + `.summary.json` · `cardiac_kg_summary.json`, `crosswalk_check.json` · `train.csv` / `test.csv` **not on the laptop** (train was downloaded and decoded on Colab, inside the B0/B1 job) · **`models/nightingale_b0_b1/`** and **`models/nightingale_b1_asked/`** (EXP-018: `b1_aug/`, `b1_asked_aug/`, 2026-09-25), each with the zip it came in; the first is the owner's Colab bundle (3 MB: the three models, `metrics.json`, `run.json`, both splits' summaries). The docs and `train_baselines.py` expect `models/b0_b1/`; moving it waits for the user's OK |
...
|---|---|---|---|
| 2026-09-25 | the user + Claude | **XGBoost's seeds 43–46 go to Colab; 2c starts.** The user chose Colab for the seeds (D-7) and asked for 2c meanwhile. `notebooks/colab_b1_seeds.ipynb` runs the unchanged `scripts/train_baselines.py` 13 times (seeds 43–46 of the three XGBoost arms, and seed 42 of B1-XGB as a reproducibility check), skipping finished runs if the session drops; `docs/11` §4.3 is its runbook, EXP-020 its log entry. Nothing was trained on the laptop | → pending |
| 2026-09-25 | the user + Claude | **Why Windows blocks scikit-learn: Smart App Control.** The user asked why. Read-only checks found Smart App Control On, refusing 3 of scikit-learn's 69 compiled files it has no reputation for (numpy's and XGBoost's unsigned files load). The owner chose to leave it on; the 9 tests that need scikit-learn now skip on that error only, so the local suite passes (665 passed, 18 skipped) and CI still runs them | `34d9f40` |
| 2026-09-25 | Claude | **EXP-019: every system at 100%, 50% and 25% evidence; A-7 settled; the configured ranker changes.** Scored only after the masks' digests were committed (`0f47d67`). The original B1 misses the safety targets at 25% (must-not-miss recall@3: XGB 0.877, LR 0.925; dangerous false negatives 0.059, 0.024); every model trained on masked copies meets them (≥ 0.996, ≤ 0.001), and the "asked" channel adds a small significant gain at 25% (top-1 0.931 → 0.946 for LR). Three findings for the team: the augmented B1 saturates top-3 and must-not-miss recall again at 25%, so which B1 the reduced-evidence claims compare against decides them (errata item 17); the red-flag layer collapses at reduced evidence (sensitivity 0.135 at 25%); and **EXP-018's "overconfident on short input" is narrowed**: calibrated against DDXPlus on masked validate, near-certain on hand-written cases. The configured ranker becomes B1-LR′+aug; the golden cases hold with it. A-7 from a three-judge panel's majority (precision 0.848, strict 0.512). **Three independent verifiers** (the first attempt died at the spend limit) re-ran the scoring byte for byte and found errors, fixed before commit: the panel judged **104** pairs, not 96; it was **not blind** to firing rates as written, since the auto-loaded project notes state some; "every rule needs a combination of findings" was false (the pulmonary-edema rule fires on one); B2 is not best on Precision@3 at 100%; "LR and XGBoost are equivalent" was overstated (XGBoost keeps a small edge); and the "correction" of EXP-018 is a narrowing. They also found that B1-LR's miss of the must-not-miss target at 25% depends on the mask rule (it met it under EXP-017's token rule), now said in EXP-019 | `b8c595a` |
| 2026-09-25 | the user + Claude | **`docs/05` amendment 3 approved and applied (v1.3); A-5 kept at ≥ 8; EXP-019's masks recorded.** The user relayed the team's approval of amendment 3, delegated A-7 to Claude ("keep whatever red flags you find appropriate") and took the recommendation on A-5. The approved text was applied verbatim, dates filled, with one passage added to §9's justification: what was scored between the proposal and its approval (EXP-008, EXP-018). Four independent reviewers checked it: faithful to the proposal and within scope; but **Claude had written "A-9 approved with the amendment"**, which the approval never said — reverted, A-9 is open again (as is the proposal's decision 6, tuning parity, now T-6) — and had mislabelled the retrained models in §9; both corrected before commit. The fourth reviewer found contradictions **inside the approved wording** (the worst: §4 "B1-XGB is never retrained under the same label" against §6 "the gap is closed when B1-XGB is next retrained"), sent to the team as errata (E-1), not silently fixed. The validation masks reproduce the proposal's digest `e99a7a8fbf792675`; with the asked-set digest `af3a357f6454838a` they are recorded in EXP-019 before any system is scored on them. A ~~blind~~ three-judge panel settled A-7's pairs (next commit; not fully blind to firing rates, as that row says) | `0f47d67` |
| 2026-09-25 | the user + Claude | **EXP-018 evaluated: R-18's atrial-fibrillation answer is gone.** The owner ran the Colab retrain at `4583e91` and unzipped it (into `models/nightingale_b1_asked/`, not the `models/b1_*` the runbook named: used where it is, nothing moved). Claude reproduced every validate score on the laptop (within 5 × 10⁻¹²) and ran the checks that need no mask. Original XGBoost answered AF to all four golden cases; no retrained model does, and the masked copies alone are enough. Full evidence is unchanged. Two new findings: the retrained models are overconfident on short input (GC-001: unstable angina 0.98–1.00, MI near 0), which 2b must calibrate for, and GC-003's dissection slips to 4th under the channel models (2c). The channel's own value waits for masked validate (amendment 3); the configured ranker stays B1-LR. R-18 re-scored 20 → 12 | `5ff4573` |
| 2026-09-25 | the user + Claude | **2d ✅ (EXP-008): the red-flag rules and safety layer v1.** The user chose Colab for EXP-018 and asked for 2d meanwhile. Every rule now follows a published pattern and every must-not-miss condition has one. Designing them turned up two DDXPlus traps, disclosed in EXP-008: "tearing" pain is listed for 76% of pneumothorax and Boerhaave patients, and rest pain for 56% of pneumothorax patients. Two corrections of framing: the share of all patients flagged barely moved (59% → 58%) and was never the right measure, since half of DDXPlus's patients have a must-not-miss condition; on the others the burden fell 31% → 25%. And the PE rule had never checked the symptom its own reason named. Found while writing the entry: the PE flags on pulmonary-edema patients come mostly from calf pain, not one-sided swelling as first written; corrected before commit. The owner's EXP-018 bundles arrived during the work | `9986024` |
