...
**Last updated:** 2026-09-25 · by Claude (2d ✅, EXP-008; EXP-018 evaluated: the retrained B1 no longer answers AF to short input) · **Last verified commit:** `5ff4573`
...
| **A-9** | How the encoder's "asked" channel treats the questions a patient did **not** list when a mask shortens their history (`docs/02` §9; the amendment-3 proposal leaves it to the encoder) | asked at the same share as the listed ones (current setting) · never asked | **The current setting.** Counting only the kept listed questions as asked would leak the label, since DDXPlus lists a default answer ("did not travel") only for conditions that ask it, and would leave masked patients almost no denials. The listed tokens and the proposal's digest are the same either way | How EXP-018's models, and any `+aug` model, see a masked patient |
