...
**Last updated:** 2026-09-25 · by Claude (`docs/05` amendment 3 approved by the team and applied, v1.3; errata found by review, for the team; EXP-019's masks recorded before scoring) · **Last verified commit:** `5ff4573`
...
| **Next action** | **Claude, now:** EXP-019, every system at 100%, 50% and 25% evidence on the approved masks (their digests recorded in `docs/08` before scoring); then A-7's mapping from the blind panel. **Then, unblocked:** 2c, the fusion (D-10 settled); 1e, the FastAPI endpoints; 2b's calibration, including short input. **The team:** **E-1**, the errata to amendment 3 (a contradiction and gaps in the approved wording; settle before 2c or H6 is judged) · **A-9** (sent with the amendment, not named in its approval) · **T-6**, tuning parity · **A-8** and the clinical review of the 17 representative answers (gate 1's last item before anything deep is trained). **The owner:** place the training run for XGBoost's missing seeds 43–46 (amendment 3 §6; D-7). **The user:** GC-005; whether the bundles move to `models/b0_b1/`. |
...
| **Health** | 667 tests (598 + 3 crosswalk entries + 66 for 2d's rules and safety layer). Locally 658 pass and the 9 live Neo4j tests skip unless `NEO4J_TEST_DOTENV=1` (with it all 9 passed against Aura on 2026-09-19 and again on 2026-09-24) · **CI green on GitHub at `5ff4573`**: 629 passed, 38 skipped (CI has no `data/` or `models/`) · the Colab-trained B0/B1 models reproduce their validate metrics to within 3 × 10⁻¹² |
...
stays B1-LR. The bundles are in `models/nightingale_b1_asked/`. `EvidenceEncoder(asked_channel=True)`
...
channel exists yet, so the configured ranker is unchanged. **🎯 The W3 walking skeleton is
reached (2026-09-24):**
...
**Task (2026-09-25):** EXP-019 scoring, then A-7. The amendment, the decisions and the masks'
digests are committed (this commit); nothing has been scored on the masks.

**Remaining:** run `scripts/evaluate_reduced_evidence.py` (2–3 min, scoring only), log EXP-019,
verify its claims independently; implement A-7 from the panel's majority votes.

**If interrupted:** the scoring is re-runnable; it refuses to run unless both recorded digests
reproduce.
...
- **R-18, the "asked" channel** (the deep-ranker plan's R2) · code ✅ 2026-09-24; retrain on **Colab**
  ...
  on short input; masked validate waits for amendment 3 — `5ff4573`. `EvidenceEncoder(asked_channel=True)`: `E_nn=unasked`
