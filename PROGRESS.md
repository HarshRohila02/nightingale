...
| **Health** | 683 tests (667 + 16 for amendment 3's digests, A-7 and the configured ranker's golden cases). Locally 665 pass, **9 fail because Windows' Application Control now blocks scikit-learn** (§7; CI runs them), and the 9 live Neo4j tests skip unless `NEO4J_TEST_DOTENV=1` (with it all 9 passed against Aura on 2026-09-19 and again on 2026-09-24) · **CI green on GitHub at `b8c595a`**: 640 passed, 43 skipped (CI has no `data/` or `models/`) · the Colab-trained B0/B1 models reproduce their validate metrics to within 3 × 10⁻¹² |
